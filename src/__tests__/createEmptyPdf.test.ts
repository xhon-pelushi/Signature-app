...
async function blobToArrayBuffer(maybeBlob: any): Promise<ArrayBuffer> {
  ...
  if (maybeBlob && typeof maybeBlob === 'object' && typeof (maybeBlob as any).size === 'number' && typeof (maybeBlob as any).slice === 'function' && typeof Response !== 'undefined') {
    try {
...
describe('createEmptyPdf', () => {
  ...
  it('adds ellipsis when truncated by bodyMaxLines with ellipsisOverflow', async () => {
    const text = Array.from({ length: 40 }, (_, i) => `Paragraph ${i + 1} lorem ipsum dolor sit amet.`).join('\n');
