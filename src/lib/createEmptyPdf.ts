...
export async function createEmptyPdf(title = "Sample Document", options: CreateEmptyPdfOptions = {}) {
  const { pages = 1, size = "LETTER", orientation = "portrait", footer = true, subject = "Sample PDF", author = "SignatureApp", keywords = ["SignatureApp", "Sample", "PDF"], guides = false, watermark, titleColor = [0.2, 0.2, 0.2], bodyText = "This is a sample PDF generated for testing the viewer.", bodyColor = [0.3, 0.3, 0.3], titleAlign = "left", footerDateFormat = "iso", titleFontSize = 24, bodyFontSize = 12, backgroundColor, footerAlign = "left" } = options;
  ...
  for (let i = 0; i < pages; i++) {
    ...
    if (footer) {
      ...
      if (footerAlign === "center") {
        ...
        x = width - 72 - font.widthOfTextAtSize(footerText, footerSize);
      }
