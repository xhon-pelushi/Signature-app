    ...
    for (const f of pageFields) {
      ...
      switch (f.type) {
        ...
        case "text": {
          ...
          page.drawText(f.label ?? "Text", { x: x + 4, y: y + h / 2 - 5, size: Math.max(8, Math.min(12, h * 0.4)), font, color: rgb(0.2, 0.2, 0.2) });
          break;
        }
        case "checkbox": {
          const size = Math.min(w, h);
          page.drawRectangle({ x, y, width: size, height: size, borderColor: rgb(0.4, 0, 0.6), borderWidth: 1 });
          break;
        }
        case "date": {
          page.drawRectangle({ x, y, width: w, height: h, borderColor: rgb(1, 0.5, 0), borderWidth: 1 });
          page.drawText("Date", { x: x + 4, y: y + h / 2 - 5, size: Math.max(8, Math.min(12, h * 0.4)), font, color: rgb(1, 0.5, 0) });
          break;
        }
        default:
          break;
