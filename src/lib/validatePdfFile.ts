    ...
    try {
      ...
      if (versionMatch) {
        ...
        if (version < 1.0 || version > 2.0) {
          warnings.push(`Unusual PDF version detected: ${versionMatch[1]}`);
        }
      }
    } catch (err) {
      warnings.push(`Unable to read file header for sniffing: ${err instanceof Error ? err.message : 'unknown error'}`);
    }
  }
